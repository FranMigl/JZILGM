!   grpnet_poisson.f90 - group elastic net (poisson)
...
!   beta = initial vector of coefficients (P)
!   off = offset vector (N)
...
SUBROUTINE grpnet_poisson(nobs, nvars, x, y, w, beta, off, ngrps, gsize, pw, alpha, &
                          nlam, lambda, lmr, penid, gamma, eps, maxit, &
                          standardize, intercept, ibeta, betas, mus, iters, &
...
! --------------- ARGUMENTS --------------- !
    ...
    DOUBLE PRECISION x(nobs, nvars), y(nobs), w(nobs), beta(nvars), off(nobs), pw(ngrps)
    DOUBLE PRECISION alpha, lambda(nlam), lmr, gamma, eps, ibeta(nlam)
    DOUBLE PRECISION betas(nvars, nlam), mus(nobs, nlam), edfs(nlam), devs(nlam), nulldev
! --------------- ARGUMENTS --------------- !
...
! --------------- LOCAL DEFINITIONS --------------- !
    ...
    DOUBLE PRECISION r(nobs), difbeta(nvars), difibeta
...
! --------------- GENERATE LAMBDA --------------- !
    IF (maxlam <= macheps) THEN
        ...
        ! calculate deviance !
        CALL grpnet_poisson_dev(nobs, y, mu, w**2, devs(i))
        ! calculate deviance !
...
! --------------- POST PROCESSING --------------- !
    ...
    ELSE
        mu = EXP(off)
    END IF
    CALL grpnet_poisson_dev(nobs, y, mu, w**2, nulldev)
...
SUBROUTINE grpnet_poisson_dev(nobs, y, mu, wt, dev)
    IMPLICIT NONE
    INTEGER nobs, i
    ...
    DO i=1,nobs
        IF (y(i) > 0) THEN
            dev = dev + 2.0D0 * wt(i) * ( y(i) * log(y(i) / mu(i)) - (y(i) - mu(i)) )
        ELSE
            dev = dev + 2.0D0 * wt(i) * mu(i)
        END IF
    END DO
END SUBROUTINE
